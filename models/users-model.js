...
UsersSchema.pre("save", function (next) {
	...
	if (this.isNew || user.code == undefined) {
		user.code = user.firstName.slice(0, 1) + user.lastName.slice(0, 3) + "-" + randomString(6, 16);
		next();
	} else {
		return next();
	}
...
UsersSchema.post("save", function (doc) {
	if (this.isNew || doc.auto_increment_id == 0 || doc.auto_increment_id == undefined) {
		UsersModel.findOne({})
			.sort("-auto_increment_id")
			.limit(1) // give me the max
			.exec(function (err, user) {
				UsersModel.updateOne({ _id: ObjectId(doc._id) }, { $set: { auto_increment_id: user.auto_increment_id + 1 } }, function (error, user) {
					if (error) {
					}
				});
			});
